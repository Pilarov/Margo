...
| # | ADR | Что | Зачем |
|---|---|---|---|
| 3 | **ADR-013** | завершить window: S2/S3 в `retriever.ts` (счётчик `k` уже в телеметрии, `recall.min=30` зафиксирован замером) | recall без жёстких топов |
| 4 | **ADR-007 §S1** | lexical-канал + type-recall в S1 (RRF) | закрыть pnpm/gRPC gap (TD-005) |

Не зависят от ADR-012 — используют текущий embedding.

**Depends on**: Этап 2.0 (P1, P5) — иначе критерий не измерим.
**Exit criteria**: falsification ADR-013 выполнен — `recall@10 ≥ 0.83` и `p99 ≤ +10%` против `qa/baseline-2026-09-24.json`; TD-005 закрыт: `q-package-manager` перестал быть нулевым (был 0.00), `q-backend` не ниже 0.67, оба значения приведены в отчёте прогона; гейт ADR-010 — PASS.
...
`retrieve()` и `searchMemories()` ≤ 100 строк каждый без SQL и скоринговых констант; публичные контракты
неизменны (`similarity`, `score`/`source`, metadata-ключи, экспортированные хелперы — source-inspection тесты зелёные).
