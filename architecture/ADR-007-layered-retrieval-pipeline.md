...
**Revision**: 2026-09-24 — §S1 fusion contract (стратегия фузии, лексический канал, маппинг скоров); Falsification переобъявлены по ADR-010 §8
**Scope**: 2026-09-24 — **сужен ADR-015**: Margo — сервис памятей, поэтому реализуется один вход (`searchMemories`); второй вход (`retrieve()`, документы) вырезается. Слои S0–S3, каналы и правила этого ADR относятся к memory-пути; документные модули карты (`rerank/cross-encoder`, `delivery/pack`, документный semantic-канал) сняты.
...
### S1 fusion contract (поправка 2026-09-24)

Фузия — единственное место, где встречаются каналы, и её форма решает, что окажется в топе. Замерено до
реализации: `research/2026-09-24-fusion-semantic-lexical.md`, данные `reviews/td005/fusion-*.json`.

1. **Стратегия фузии конфигурируема** — `retrieval.fusion = { strategy: "rrf" | "top-up", rrfK, semanticWeight, lexicalWeight, keep }`. Базовая — **RRF с равными весами** (`rrfK = 60`), как в §Decision; `top-up(keep)` — измеренная альтернатива. Новые стратегии добавляются так же, как селекторы окна (реестр + конфиг), — это и есть запас под «интереснее для слоя».
2. **Лексический канал S1 — OR-семантика FTS**: `to_tsvector('english', content) @@ to_tsquery(...)` по термам запроса, порядок `ts_rank_cd`. AND-семантика (`plainto_tsquery`) отвергнута замером: 0 совпадений у 30 из 54 вопросов. Канал закрывает TD-005 (`q-package-manager` был 0.00).
3. **RRF — фузия по рангам, не по скорам** (шкалы несопоставимы: косинус 0.9–1.15 против `ts_rank_cd`). Веса = доверие каналу, 1:1 = равное. Измерено: лексический вес 0.3/0.5/0.7 даёт одинаковый результат, перекос 0.7:1.0 в пользу лексики стоит 4 вопроса — рычаг не вес, а разрешено ли переупорядочивание головы.
4. **Маппинг скоров обязателен**: S2/S3 пересчитывают ранжирование из `similarity` (`similarity*0.7 + temporal*0.3`), поэтому кандидат, пришедший только через лексику, обязан получить `similarity`, отображённый по рангу в диапазон семантического списка; иначе фузия не доживает до доставки.
5. **Потолок объединения (измерено)**: на top-10 каналы комплементарны как 37 / 6 / 8 / 3 (оба / только семантика / только лексика / никто) → **51/54 вопросов**. 54/54 каналами не достигается: у трёх «дырок» reference-памяти уже найдены семантикой (ранги 16/37/47) — их надо продвигать на S2/S3 и окном (ADR-013), а не искать новым каналом.
6. **Адаптивный роутер не вводится** (пока): разделяющего признака нет — `top_similarity` (0.90–1.15) и число лексических совпадений перекрываются между «лексика спасает» и «и так в порядке». Гибкость дают: конфиг стратегии, фузия на уровне реранка при рефакторинге S2 (Этап 5) и тюнер ADR-011.
...
- **S1 lexical закрывает TD-005**: `q-package-manager` ≠ 0 и `q-backend` ≥ 0.67. Метрика: `recall@10` по этим вопросам из `per_question`. Baseline: `qa/baseline-2026-09-24.json`. Статистика: одиночный прогон (сьют детерминирован — расхождение двух прогонов 0.000 п.п.). Протокол: `python3 scripts/benchmark/run.py --suite retrieval --k 10`. Допуск: `recall@10` не ниже baseline −2 п.п. Направление: выше лучше.
- **Фузия конфигурируема и обратима**: `retrieval.fusion.strategy` (`rrf` ↔ `top-up`) и веса переключаются строкой конфига без правок кода; возврат к semantic-only воспроизводит baseline `recall@10 = 0.787` (±2 п.п.).
- **Каналы независимы**: отключение лексического канала не ломает семантический (прогон даёт baseline-уровень) и не падает с ошибкой.
- **p99 latency S0+S1 ≤ 150ms** при пуле ≤ 1000 памятей.
- **type-recall не теряется**: требует групповой метрики по type-вопросам, которой в сьюте нет (открытый пункт) — до её появления критерий неgradable и не закрывается.
- **graph-канал даёт прирост**: требует S1 graph-канала, которого в memory-пути пока нет (`enrichWithRelations` дописывает связи, но не является каналом recall) — критерий неgradable до его появления, а не снят.
- ~~recall@10 ≥ 0.90 на 21 вопросе~~ — **снято**: абсолютное число без baseline-файла и протокола критерием не является (ADR-010 §8 п.1).

Если после рефакторинга `recall@10` падает ниже baseline −2 п.п. или p99 S0+S1 выходит за 150ms — решение пересмотреть.
