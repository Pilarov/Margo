...
**Status**: Proposed
**Date**: 2026-09-08
**Revision**: 2026-09-24 — §8 (достоверность публикуемых чисел)
...
### 8. Достоверность публикуемых чисел (поправка 2026-09-24)

Проверка исполнением (TD-011): `user_id`/`session_id` всегда включают scoped keyword-merge, после чего ответ пишет `vector_ms: 0, embed_ms: 0`, оставляя их в `total_ms` (медиана 116 ms против 25 ms показанных стадий). Правила:

1. **Разбивка складывается**: сумма публикуемых стадий равна `total_ms`; работа вне `searchMemories` (merge, pending-overlay) публикуется отдельными полями.
2. **Merge ≠ деградация**: `fallback: "lexical"` — выполненный scoped keyword-merge, а не отказ векторного пути; имя поля обязано их различать.
3. **У каждой стадии воронки — свой знаменатель** (`samples` рядом с `avg_in/avg_out`): стадия есть не во всех сэмплах, сравнимы только сопоставимые знаменатели.
4. **Сигнал критерия воспроизводим вне процесса** (ADR-010 §8): число, на которое опирается критерий, восстанавливается из артефакта после рестарта, а не живёт только в in-memory коллекторе.

**Falsification**: для любого ответа сумма стадий = `total_ms` (в пределах округления); у каждой стадии воронки есть знаменатель; число из критерия воспроизводится из артефакта после рестарта. Отклонено: оставить как есть (числа противоречат друг другу) и перенести целиком в ADR-010.
